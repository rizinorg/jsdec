...
// old graph has base64 encoded comments.
function _fixup_old_graph(graph) {
    graph[0].blocks.forEach((block) => {
        block.ops.forEach((instr) => {
            instr.comment = instr.comment ? atob(instr.comment) : undefined;
        });
    });
    return graph;
}
